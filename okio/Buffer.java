...
public final class Buffer implements Cloneable, BufferedSink, BufferedSource {
  ...
  private boolean rangeEquals(Segment paramSegment, int paramInt1, ByteString paramByteString, int paramInt2, int paramInt3) {
    ...
    //   32: if_icmpne -> 62
    //   35: aload_1
    //   36: getfield next : Lokio/Segment;
    //   39: astore #9
    //   41: aload #9
    //   43: getfield data : [B
    //   46: astore #7
  ...
  public Buffer clone() {
    ...
    if (this.size != 0L) {
      ...
      for (segment3 = this.head.next; segment3 != this.head; segment3 = segment3.next)
        buffer.head.prev.push(new Segment(segment3)); 
  ...
  boolean rangeEquals(long paramLong, ByteString paramByteString) {
    ...
    //   47: lload_1
    //   48: ladd
    //   49: invokevirtual getByte : (J)B
    //   52: aload_3
    //   53: iload #7
    //   55: invokevirtual getByte : (I)B
    //   58: if_icmpne -> 26
    //   61: iinc #7, 1
    //   64: goto -> 32
    //   67: iconst_1
  ...
  public int readInt() {
    ...
    m = n + 1;
    i = (i & 0xFF) << 24 | (k & 0xFF) << 16 | (b & 0xFF) << 8 | arrayOfByte[n] & 0xFF;
    this.size -= 4L;
    if (m == j) {
      this.head = segment.pop();
      SegmentPool.recycle(segment);
  ...
  public long readLong() {
    ...
    i = k + 1;
    l1 = (l2 & 0xFFL) << 56L | (l3 & 0xFFL) << 48L | (l1 & 0xFFL) << 40L | (l4 & 0xFFL) << 32L | (l5 & 0xFFL) << 24L | (l6 & 0xFFL) << 16L | (l7 & 0xFFL) << 8L | arrayOfByte[k] & 0xFFL;
    this.size -= 8L;
    if (i == j) {
      this.head = segment.pop();
      SegmentPool.recycle(segment);
  ...
  public int select(Options paramOptions) {
    ...
    //   69: invokespecial rangeEquals : (Lokio/Segment;ILokio/ByteString;II)Z
    //   72: ifeq -> 98
    //   75: aload_0
    //   76: aload #5
    //   78: invokevirtual size : ()I
    //   81: i2l
    //   82: invokevirtual skip : (J)V
    //   85: goto -> 17
    //   88: astore_1
  ...
  public Buffer writeUtf8(String paramString, int paramInt1, int paramInt2) {
    ...
    label42: while (paramInt1 < paramInt2) {
      ...
      if (c < '') {
        Segment segment = writableSegment(1);
        byte[] arrayOfByte = segment.data;
        i = segment.limit - paramInt1;
        int j = Math.min(paramInt2, 8192 - i);
        arrayOfByte[i + paramInt1] = (byte)(byte)c;
        paramInt1++;
        while (true) {
          if (paramInt1 < j) {
...
/* Location:              D:\Code\Crack\dex-tools-2.1-20190905-lanchon\zspns-pcgw-P36417A-0429-dex2jar.jar!\okio\Buffer.class
