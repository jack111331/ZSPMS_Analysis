...
import org.jar.ptr.PtrFrameLayout;
import org.jar.ptr.PtrUIHandler;
import org.jar.ptr.indicator.PtrIndicator;
import org.jar.ptr.util.PtrLocalDisplay;
      ...
      if (b < j) {
        ...
        if (this.mIsInLoading) {
          ...
          if (f == 0.0F) {
            ...
            float f4 = (1.0F - this.mInternalAnimationFactor) * b / j;
            float f5 = this.mInternalAnimationFactor;
            if (f == 1.0F || f >= 1.0F - 1.0F - f5 - f4) {
              paramCanvas.translate(f1, f2);
              storeHouseBarItem.setAlpha(this.mBarDarkAlpha);
            } else {
              if (f > f4)
                f3 = Math.min(1.0F, (f - f4) / this.mInternalAnimationFactor); 
              ...
              matrix.postTranslate(f1 + f6 * f4, f2 + f5 * f4);
...
/* Location:              D:\Code\Crack\dex-tools-2.1-20190905-lanchon\zspns-pcgw-P36417A-0429-dex2jar.jar!\org\jar\ptr\header\StoreHouseHeader.class
