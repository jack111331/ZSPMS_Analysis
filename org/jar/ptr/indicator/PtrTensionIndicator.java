...
public class PtrTensionIndicator extends PtrIndicator {
  ...
  private float offsetToTarget(float paramFloat) {
    ...
    Math.pow((Math.max(0.0F, Math.min(paramFloat - this.mOneHeight, this.mOneHeight * 2.0F) / this.mOneHeight) / 4.0F), 2.0D);
    paramFloat = this.mOneHeight;
    paramFloat = this.mOneHeight;
    return 0.0F;
  }
  ...
  public float getOverDragPercent() {
    return isUnderTouch() ? this.mCurrentDragPercent : ((this.mReleasePercent <= 0.0F) ? (getCurrentPosY() * 1.0F / getOffsetToKeepHeaderWhileLoading()) : (this.mReleasePercent * getCurrentPosY() / this.mReleasePos));
  }
  ...
  protected void processOnMove(float paramFloat1, float paramFloat2, float paramFloat3, float paramFloat4) {
    ...
    double d1 = (Math.max(0.0F, Math.min(paramFloat4 - this.mOneHeight, this.mOneHeight * 2.0F) / this.mOneHeight) / 4.0F);
...
/* Location:              D:\Code\Crack\dex-tools-2.1-20190905-lanchon\zspns-pcgw-P36417A-0429-dex2jar.jar!\org\jar\ptr\indicator\PtrTensionIndicator.class
