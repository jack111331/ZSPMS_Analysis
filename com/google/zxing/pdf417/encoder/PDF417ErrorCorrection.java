...
final class PDF417ErrorCorrection {
  private static final int[][] EC_COEFFICIENTS = new int[][] { { 27, 917 }, { 522, 568, 723, 809 }, { 237, 308, 436, 284, 646, 653, 428, 379 }, { 
  ...
  static String generateErrorCorrection(CharSequence paramCharSequence, int paramInt) {
    ...
    for (byte b = 0; b < j; b++) {
      ...
      int m = i - 1;
      int k = (c + arrayOfChar[m]) % 929;
      while (m > 0) {
        int n = EC_COEFFICIENTS[paramInt][m];
        arrayOfChar[m] = (char)(char)((arrayOfChar[m - 1] + 929 - n * k % 929) % 929);
        m--;
      } 
      arrayOfChar[0] = (char)(char)((929 - k * EC_COEFFICIENTS[paramInt][0] % 929) % 929);
    } 
    paramCharSequence = new StringBuilder(i);
    for (paramInt = i - 1; paramInt >= 0; paramInt--) {
      if (arrayOfChar[paramInt] != '\000')
        arrayOfChar[paramInt] = (char)(char)(929 - arrayOfChar[paramInt]); 
  ...
  static int getRecommendedMinimumErrorCorrectionLevel(int paramInt) throws WriterException {
    if (paramInt > 0) {
      if (paramInt <= 40)
        return 2; 
      if (paramInt <= 160)
...
/* Location:              D:\Code\Crack\dex-tools-2.1-20190905-lanchon\zspns-pcgw-P36417A-0429-dex2jar.jar!\com\google\zxing\pdf417\encoder\PDF417ErrorCorrection.class
