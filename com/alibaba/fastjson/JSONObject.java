...
public class JSONObject extends JSON implements Map<String, Object>, Cloneable, Serializable, InvocationHandler {
  ...
  public Object clone() {
    ...
    if (this.map instanceof LinkedHashMap) {
      hashMap = new LinkedHashMap<String, Object>(this.map);
    } else {
      hashMap = new HashMap<String, Object>(this.map);
    } 
  ...
  public JSONArray getJSONArray(String paramString) {
    ...
    return (paramString instanceof JSONArray) ? (JSONArray)paramString : ((paramString instanceof String) ? (JSONArray)JSON.parse(paramString) : (JSONArray)toJSON(paramString));
  }
  
  public JSONObject getJSONObject(String paramString) {
    paramString = (String)this.map.get(paramString);
    return (paramString instanceof JSONObject) ? (JSONObject)paramString : ((paramString instanceof String) ? JSON.parseObject(paramString) : (JSONObject)toJSON(paramString));
  }
  ...
  public <T> T getObject(String paramString, TypeReference paramTypeReference) {
    paramString = (String)this.map.get(paramString);
    return (T)((paramTypeReference == null) ? paramString : TypeUtils.cast(paramString, paramTypeReference.getType(), ParserConfig.getGlobalInstance()));
  }
  ...
  public <T> T getObject(String paramString, Type paramType) {
    return (T)TypeUtils.cast(this.map.get(paramString), paramType, ParserConfig.getGlobalInstance());
  }
      ...
      if (stringBuilder.getReturnType() != void.class) {
        ...
        if (paramObject == null) {
          ...
          if (paramObject.startsWith("get")) {
            object1 = paramObject.substring(3);
            if (object1.length() != 0) {
              paramObject = new StringBuilder();
              paramObject.append(Character.toLowerCase(object1.charAt(0)));
              paramObject.append(object1.substring(1));
...
/* Location:              D:\Code\Crack\dex-tools-2.1-20190905-lanchon\zspns-pcgw-P36417A-0429-dex2jar.jar!\com\alibaba\fastjson\JSONObject.class
