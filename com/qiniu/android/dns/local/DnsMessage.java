...
public final class DnsMessage {
  ...
  public static Record[] parseResponse(byte[] paramArrayOfbyte, int paramInt, String paramString) throws IOException {
    ...
    if (i == paramInt) {
      ...
      if (i != 0 && paramInt != 0) {
        ...
        dataInputStream.readUnsignedShort();
        readQuestions(dataInputStream, paramArrayOfbyte, i);
        return readAnswers(dataInputStream, paramArrayOfbyte, paramInt);
      } 
  ...
  private static Record[] readAnswers(DataInputStream paramDataInputStream, byte[] paramArrayOfbyte, int paramInt) throws IOException {
    ...
    byte b = 0;
    while (paramInt > 0) {
      arrayOfRecord[b] = readRecord(paramDataInputStream, paramArrayOfbyte);
      b++;
      paramInt--;
    } 
  ...
  private static void writeDomain(OutputStream paramOutputStream, String paramString) throws IOException {
    ...
    for (byte b = 0; b < i; b++) {
      byte[] arrayOfByte = IDN.toASCII(arrayOfString[b]).getBytes();
...
/* Location:              D:\Code\Crack\dex-tools-2.1-20190905-lanchon\zspns-pcgw-P36417A-0429-dex2jar.jar!\com\qiniu\android\dns\local\DnsMessage.class
